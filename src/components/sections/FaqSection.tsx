...
function PlusIcon() {
  return (
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path d="M12 5V19M5 12H19" stroke="#FDFDFD" strokeWidth="2" strokeLinecap="round" />
    </svg>
  );
}
...
function MinusIcon() {
  return (
    <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
      <path d="M5 12H19" stroke="#FDFDFD" strokeWidth="2" strokeLinecap="round" />
    </svg>
  );
}

function FaqSection() {
  const [openIds, setOpenIds] = useState<Set<number>>(new Set([faqData[0]!.id]));
  ...
  return (
    <section className="bg-bg-darkest py-10 px-4 lg:py-20 lg:px-30 flex flex-col items-center gap-6 lg:gap-12">
      ...
      <div className="flex flex-col gap-4 w-full lg:flex-row lg:justify-between lg:items-end">
        ...
        <p className="text-sm font-medium text-text-secondary lg:text-lg lg:text-right">
          Everything you need to<br className="hidden lg:block" /> know — all in one place.
        </p>
      </div>
        ...
        <div className="flex flex-col gap-7 flex-1">
          {faqData.flatMap((item, index) => [
            <div key={item.id} className="flex flex-col gap-4">
              ...
              <div
                className={`overflow-hidden transition-all duration-300 ease-in-out ${
                  openIds.has(item.id) ? 'max-h-48 opacity-100' : 'max-h-0 opacity-0'
                }`}
              >
            ...
            ...(index < faqData.length - 1
              ? [<div key={`sep-${item.id}`} className="h-px bg-border w-full" />]
              : []),
          ])}
        </div>
        ...
        >
          <div className="flex flex-col gap-1">
            <h3 className="text-display-sm font-bold text-white tracking-tight leading-snug lg:text-4xl lg:leading-tight">
              Let's talk it<br className="hidden lg:block" /> through
            </h3>
